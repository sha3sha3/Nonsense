        ...
        if (Tile && Tile->GetCoordinates() == Coord && Tile->GetTileType() == ETileType::Empty)
        {
            ...
            if (UnitClass)
            {
                ...
                if (NewUnit)
                {
                    ...
                    NewUnit->TileX = Tile->TileX;
                    NewUnit->TileY = Tile->TileY;
...
void ACommanderOfFate::MoveSelectedUnitTo(FVector2D TileCoord)
{
    ...
    for (AActor* Actor : Tiles)
    {
        ...
        if (Tile && Tile->GetCoordinates() == TileCoord && Tile->GetTileType() == ETileType::Empty)
        {
            ...
            SelectedUnit->TileX = Tile->TileX;
            SelectedUnit->TileY = Tile->TileY;
...
void ACommanderOfFate::HighlightMovementTiles()
{
    ...
    for (AActor* Actor : Tiles)
    {
        ...
        if (Tile && Tile->GetTileType() == ETileType::Empty)
        {
            int MoveX=FMath::Abs(Tile->TileX - SelectedUnit->TileX);
            int MoveY=FMath::Abs(Tile->TileY - SelectedUnit->TileY);
            int Range  = SelectedUnit->GetMovementRange();
            if (MoveX <= FMath::Abs(Range-MoveY)
                && MoveY <= FMath::Abs(Range-MoveX))
            {
    ...
    for (APlayerUnit* AIUnit : AIUnits)
    {
        ...
        if (ClosestTarget && SelectedUnit->GetReachableTiles().Num() > 0)
        {
            ...
            if (AttackDist <= Range)
            {
                ...
                // Fully correct
                ClosestTarget->TakeDamage(static_cast<float>(Damage), DamageEvent, nullptr, this);
